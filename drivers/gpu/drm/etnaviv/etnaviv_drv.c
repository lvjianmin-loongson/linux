...
MODULE_PARM_DESC(enable, "Enable etnaviv (1 = enabled, 0 = disabled (default))");
	...
	if (!dev->platform_data) {
		...
		for_each_compatible_node(core_node, NULL, "vivante,gc") {
			if (!of_device_is_available(core_node))
				continue;
			DRM_INFO("%s: %s, %s\n",
				__func__, core_node->name, core_node->type);

			drm_of_component_match_add(&pdev->dev, &match,
						   compare_of, core_node);
			if (match)
				DRM_INFO("found match\n");
		}
	} else {
...
static int __init etnaviv_init(void)
{
	...
	if (!etnaviv_enable) {
		DRM_INFO("etnaviv is being disabled.\n");
		return -ENODEV;
	}
	...
	for_each_compatible_node(np, NULL, "vivante,gc") {
		DRM_INFO("device node name: %s, type: %s\n",
				np->name, np->type);

		DRM_INFO("device node full name: %s\n",
