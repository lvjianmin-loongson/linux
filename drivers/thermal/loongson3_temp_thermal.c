...
* more than 8. Loongson-3 support 8 thresholds now, 0-3 is low
* temp threshold, and 4-7 is high temp thredhold.
*/
#define MAX_NUMBER_OF_TRIPS 8
...
static int loongson3_laptop_trip_temp[MAX_NUMBER_OF_TRIPS] = {
	-30000, -32000, -34000, -36000,
...
static int loongson3_init_trip_temp(int pkg_id)
{
	...
	for(trip = 0; trip < MAX_NUMBER_OF_TRIPS; trip++){
		if (is_laptop == true)
			temp = loongson3_laptop_trip_temp[trip];
		else
...
sys_set_trip_temp(struct thermal_zone_device *tzd, int trip, int temp)
{
	...
	if (is_laptop == true)
		loongson3_laptop_trip_temp[trip] = temp;
	else
...
static inline void loongson3_enable_pkg_thres_interrupt(int pkg_id)
{
	...
	reg_hi |= (TEMP_HIGH_INT_ENABLE(0) | TEMP_HIGH_INT_ENABLE(1) |
			TEMP_HIGH_INT_ENABLE(2) | TEMP_HIGH_INT_ENABLE(3));
...
static void pkg_temp_thermal_threshold_work_fn(struct work_struct *work)
{
	...
	if (is_laptop == true) {
		...
		if (cur_temp > loongson3_laptop_trip_temp[7])
			thermal_state = 3;
		else if (cur_temp > loongson3_laptop_trip_temp[6])
			thermal_state = 2;
		else if (cur_temp > loongson3_laptop_trip_temp[5])
			thermal_state = 1;
		else
			thermal_state = 0;
...
static int pkg_thermal_cpu_offline(unsigned int cpu)
{
	...
	if (lastcpu) {
		...
		LOONGSON_TEMPINTLO_SET(pkgdev->temp_int_lo_reg, topology_physical_package_id(cpu));
		LOONGSON_TEMPINTHI_SET(pkgdev->temp_int_hi_reg, topology_physical_package_id(cpu));
	}
