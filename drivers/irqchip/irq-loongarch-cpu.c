...
{
	set_csr_excfg(ECFGF(d->hwirq));
}

static inline void mask_loongarch_irq(struct irq_data *d)
{
	clear_csr_excfg(ECFGF(d->hwirq));
}
...
{
	unsigned long pending = __lcsr_dcsrrd(LOONGARCH_CSR_EXCFG)
		& __lcsr_dcsrrd(LOONGARCH_CSR_EXST) & ECFG0_IM;
	unsigned int virq;
	int irq;
...
IRQCHIP_ACPI_DECLARE(coreintc_v1, ACPI_MADT_TYPE_COREINTC,
		NULL, ACPI_MADT_COREINTC_VERSION_V1,
