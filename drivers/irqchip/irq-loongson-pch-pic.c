...
{
	return pch_pic_priv[id] ? pch_pic_priv[id]->gsi_base : NULL;
}
...
{
	struct acpi_madt_pchintc *pch_pic_entry;
	struct fwnode_handle *irq_handle;
	pch_pic_entry = (struct acpi_madt_pchintc *)header;

	irq_handle = irq_domain_alloc_named_id_fwnode("pchintc", pch_pic_entry->id);
	if (!irq_handle) {
...
IRQCHIP_ACPI_DECLARE(pch_pic_v1, ACPI_MADT_TYPE_PCHINTC,
		NULL, ACPI_MADT_PCHINTC_VERSION_V1,
