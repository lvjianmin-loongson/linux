...
#define LS7A_DC_GPIO_IO_REG(x)		(LS7A_DC_CNT_REG_BASE + x->in_offset)
#define LS7A_DC_GPIO_CONF_REG(x)	(LS7A_DC_CNT_REG_BASE + x->conf_offset)
...
__set_direction(struct loongson_gpio_chip *lgpio, unsigned pin, int input)
{
	...
	if (!strcmp(lgpio->chip.label,"loongson,ls7a-gpio") ||
		...
		if (input)
			...
			value = 0;
		ls7a_writeb(value, (unsigned long)LS7A_GPIO_OEN_BYTE(lgpio, pin));
		return ;
	}
...
static void __set_level(struct loongson_gpio_chip *lgpio, unsigned pin, int high)
{
	...
	if (!strcmp(lgpio->chip.label,"loongson,ls7a-gpio") ||
	        ...
	        if (high)
		        ...
		        value = 0;
		ls7a_writeb(value, (unsigned long)LS7A_GPIO_OUT_BYTE(lgpio, pin));
		return;
	}
...
static int loongson_gpio_get(struct gpio_chip *chip, unsigned pin)
{
	...
	if (!strcmp(lgpio->chip.label, "loongson,loongson3-gpio") ||
		...
		return ((temp & (1 << (pin + lgpio->in_start_bit))) != 0);
	}

	if (!strcmp(lgpio->chip.label,"loongson,ls7a-gpio") ||
			!strncmp(lgpio->chip.label, "LOON0002", 8)){
		value = ls7a_readb((unsigned long)LS7A_GPIO_IN_BYTE(lgpio, pin));
                return (value & 1);
        }
			    ...
			    void __iomem *base)
{
	...
	if (!strcmp(lgpio->chip.label, "loongson,ls7a-gpio") ||
		...
		!strncmp(lgpio->chip.label, "LOON0003", 8)) {
