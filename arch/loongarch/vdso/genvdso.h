...
static inline bool FUNC(patch_vdso)(const char *path, void *vdso)
{
	...
	for (i = 0; i < sh_count; i++) {
		...
		if (strcmp(name, ".LOONGARCH.abiflags") == 0) {
			fprintf(stderr,
				"%s: '%s' already contains a '.LOONGARCH.abiflags' section\n",
				program_name, path);
			return false;
		}

		if (strcmp(name, ".loongarch_abiflags") == 0) {
			strcpy(name, ".LOONGARCH.abiflags");
			shdr->sh_type = swap_uint32(SHT_LOONGARCH_ABIFLAGS);
