...
void __init memblock_and_maxpfn_init(void)
{
	int i;
	u32 mem_type;
	u64 mem_start, mem_end, mem_size, node_id;
	...
	if (loongson_memmap) {
		...
		for (i = 0; i < loongson_memmap->nr_map; i++){
			...
			mem_start = loongson_memmap->map[i].mem_start | (node_id  << 44);
			mem_size = ((u64)loongson_memmap->map[i].mem_size) << 20;
			mem_end = (node_id  << 44) | (mem_start + mem_size);
			switch (mem_type) {
			case SYSTEM_RAM_LOW:
...
/* override of arch/loongarch/mm/cache.c: __uncached_access */
