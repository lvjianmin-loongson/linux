...
static void __init szmem(unsigned int node)
{
	...
	for (i = 0; i < loongson_memmap->nr_map; i++) {
		...
		case SYSTEM_RAM_LOW:
			...
			memblock_set_node(PFN_PHYS(start_pfn),
				PFN_PHYS(end_pfn - start_pfn), &memblock.memory, node);
			break;
	...
	if (loongson_sysconf.uma_vram_size)
		reserve_video_ram(loongson_sysconf.uma_vram_addr,
					loongson_sysconf.uma_vram_size);
}
