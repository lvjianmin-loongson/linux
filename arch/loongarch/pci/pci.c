...
static void pcibios_scanbus(struct pci_controller *hose)
{
	...
	if (cpu_guestmode)
		bridge->map_irq = ls7a_guest_pcibios_map_irq;
...
static struct pci_controller pch_pci_controller = {
...
void loongarch_pci_root_resources(struct list_head *resources)
{
	if (resources) {
...
int __init pci_controller_init(u64 pci_mem_start_addr, u64 pci_mem_end_addr,
					u64 io_port_base,
					struct pci_ops * pci_ops)
{
	...
	pch_pci_controller.pci_ops = pci_ops;
	pch_pci_controller.io_map_base = io_port_base;
	pch_pci_controller.mcfg_addr = mcfg_addr_init(0);
	register_pci_controller(&pch_pci_controller);
	return 0;
}
