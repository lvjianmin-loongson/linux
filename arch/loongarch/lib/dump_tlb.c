...
static void dump_tlb(int first, int last)
{
	...
	for (i = first; i <= last; i++) {
		...
		if (index & LOONGARCH_TLBIDX_EHINV)
			continue;
		/*
		...
		pa = entrylo0 & ~(LOONGARCH_ENTRYLO_RI | LOONGARCH_ENTRYLO_XI);
		pa = (pa << 6) & PAGE_MASK;
		pr_cont("\n\t[");
		pr_cont("ri=%d xi=%d ",
			(entrylo0 & LOONGARCH_ENTRYLO_RI) ? 1 : 0,
			(entrylo0 & LOONGARCH_ENTRYLO_XI) ? 1 : 0);
		...
		/* RI/XI are in awkward places, so mask them off separately */
		pa = entrylo1 & ~(LOONGARCH_ENTRYLO_RI | LOONGARCH_ENTRYLO_XI);
		pa = (pa << 6) & PAGE_MASK;
		pr_cont("ri=%d xi=%d ",
			(entrylo1 & LOONGARCH_ENTRYLO_RI) ? 1 : 0,
			(entrylo1 & LOONGARCH_ENTRYLO_XI) ? 1 : 0);
		pr_cont("pa=%0*llx c=%d d=%d v=%d g=%d plv=%d]\n",
			pwidth, pa, c1,
