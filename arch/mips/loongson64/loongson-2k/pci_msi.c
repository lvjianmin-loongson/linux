...
extern void mask_pic_irq(struct irq_data *data);
extern int ls2k_pic_set_affinity(struct irq_data *d, const struct cpumask *affinity,
		bool force);

struct irq_chip ls2k_msi_chip = {
		.name = "ls2k-msi",
		.irq_mask = ls2k_msi_unmask,
...
static int ls2k_msi_probe(struct platform_device *pdev)
{
	...
	irq_alloc_descs(LS2K_MSI_IRQ_BASE, LS2K_MSI_IRQ_BASE, 64, LS2K_PIC_HWIRQ_BASE);
	msi_controller->domain =  irq_domain_add_legacy(np, 64, LS2K_MSI_IRQ_BASE,
					LS2K_PIC_HWIRQ_BASE, &msi_domain_ops, msi_controller);
