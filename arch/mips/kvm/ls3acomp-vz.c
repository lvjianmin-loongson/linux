...
/*
 * LOONGSON VZ guest timer handling.
 */

/**
 * kvm_ls3acomp_should_use_htimer() - Find whether to use the VZ hard guest timer.
 * @vcpu:	Virtual CPU.
 *
 ...
 *		false otherwise.
 */
static bool kvm_ls3acomp_should_use_htimer(struct kvm_vcpu *vcpu)
{
...
static void _kvm_vz_restore_stable_stimer(struct kvm_vcpu *vcpu, u32 cause)
{
	...
	if (delta < stable_timer)
		stable_timer = stable_timer - delta;
	else
		stable_timer = 1;

	dwrite_guest_csr(LOONGSON_STABLE_TIMER_CFG, stable_timer | LOONGSON_STABLE_TIMER_CFG_EN);
	back_to_back_c0_hazard();
	write_gc0_cause(cause);
}

/**
 * kvm_ls3acomp_restore_timer() - Restore timer state.
 * @vcpu:	Virtual CPU.
 *
 * Restore soft timer state from saved context.
 */
static void kvm_ls3acomp_restore_timer(struct kvm_vcpu *vcpu)
{
					  ...
					  u32 cause)
{
	 ...
	 */
	back_to_back_c0_hazard();
	after_stable_timer = dread_guest_csr(LOONGSON_STABLE_TIMER_TICK);
	/* Stable timer count down to 48bits -1 */
	if (after_stable_timer == LOONGSON_STABLE_TIMER_MASK)
		kvm_vz_queue_irq(vcpu, MIPS_EXC_INT_TIMER);

}

/**
 * kvm_ls3acomp_acquire_htimer() - Switch to hard timer state.
 * @vcpu:	Virtual CPU.
 *
...
void kvm_ls3acomp_acquire_htimer(struct kvm_vcpu *vcpu)
{
	...
	if (!(gctl0 & MIPS_GCTL0_GT) && kvm_ls3acomp_should_use_htimer(vcpu)) {
				...
				u64 *stable_timer, u32 *out_cause)
{
	 ...
	 */
	back_to_back_c0_hazard();
	end_stable_timer = dread_guest_csr(LOONGSON_STABLE_TIMER_TICK);
	...
	*out_cause = cause;
	 ...
	 */
	if ((end_stable_timer == LOONGSON_STABLE_TIMER_MASK) && (cause & CAUSEF_TI))
		kvm_vz_queue_irq(vcpu, MIPS_EXC_INT_TIMER);
...
/**
 * kvm_ls3acomp_save_timer() - Save guest timer state.
 * @vcpu:	Virtual CPU.
 *
 * Save VZ guest timer state and switch to soft guest timer if hard timer was in
 * use.
 */
static void kvm_ls3acomp_save_timer(struct kvm_vcpu *vcpu)
{
	struct mips_coproc *cop0 = vcpu->arch.cop0;
	u32 gctl0, cause;
	...
	if (gctl0 & MIPS_GCTL0_GT) {
		...
		/*
		 * stable_timer_tick/stable_ktime_saved is calculated already
		 * at lose_timer stage, only get cause register here
		 */
		cause = read_gc0_cause();
	}
...
/**
 * kvm_ls3acomp_lose_htimer() - Ensure hard guest timer is not in use.
 * @vcpu:	Virtual CPU.
 *
	...
	switch (inst.loongson3_lscsr_format.fr) {
		...
		case 0x0:
			...
			if (vcpu->arch.gprs[rs] == 0x8) {
				...
				vcpu->arch.gprs[rd] = csr_readq(LOONGSON_CSR_FEATURES);
				/*Attention for not return some feature for guest,such as TEMPERATURE */
				vcpu->arch.gprs[rd] &= ~(LOONGSON_CSRF_TEMP | LOONGSON_CSRF_FREQ |
						LOONGSON_CSRF_FREQSCALE | LOONGSON_CSRF_DVFSV1);
				vcpu->arch.gprs[rd] |= LOONGSON_CSRF_VM;
			} else if (vcpu->arch.gprs[rs] == 0x420) {
				vcpu->arch.gprs[rd] = csr_readq(LOONGSON_CSR_OTHER_FUNC);
			} else if (vcpu->arch.gprs[rs] == 0x428) {
				++vcpu->stat.lsvz_rdcsr_cpu_temperature_exits;
				vcpu->arch.gprs[rd] = csr_readq(LOONGSON_CSR_CPUTEMP);
				...
				vcpu->arch.gprs[rd] = csr_readl(LS_CSR_ANYSEND_OTHER_FUNC) & (~CSR_OTHER_FUNC_EXT_IOI);
			} else if (vcpu->arch.gprs[rs] == 0x51c) {
				vcpu->arch.gprs[rd] = csr_readq(0x51c);
		...
		case 0x1:
			...
			if ((vcpu->arch.gprs[rs] == 0x1004) || (vcpu->arch.gprs[rs] == 0x100c)) {
				*(u32 *)data = vcpu->arch.gprs[rd];
				...
				ret = kvm_io_bus_write(vcpu, KVM_MMIO_BUS, run->mmio.phys_addr, run->mmio.len, data);
				if (!ret) {
					vcpu->mmio_needed = 0;
					er = EMULATE_DONE;
				} else {
				...
				*(u32 *)data = (1 << action);
				...
				if (!ret) {
					...
					ls3a_ext_intctl_write(vcpu->kvm,vcpu->arch.gprs[rs],4,vcpu->arch.gprs[rd]&0xffffffff);
		...
		case 0x2:
			...
			if (vcpu->arch.gprs[rs] == 0x8) {
				...
				vcpu->arch.gprs[rd] = csr_readq(LOONGSON_CSR_NODECNT) + vcpu->kvm->arch.nodecounter_offset;
		...
		case 0x3:
			...
			*(u64 *)data = vcpu->arch.gprs[rd];
			...
			if (vcpu->arch.gprs[rs] == 0x1020) {
				...
				if ((((offset & 0xff00) == 0x1600) ||
						((offset & 0xff00) == 0x1C00) ||
					(offset >= 0x0420 && offset < 0x0428) ||
					  (offset >= 0x14A0 && offset < 0x14C8))
		...
		case 0x8:
			...
			/* Ignore VZ feature for guest */
			if (vcpu->arch.gprs[rs] == 2)
				vcpu->arch.gprs[rd] &= ~LOONGSON_CPU_VZ;
			break;
			/* Stable counter read */
		case 0x9:
	...
	if (inst.co_format.co) {
		...
		case mfc_op:
			...
			if (rd == MIPS_CP0_COUNT &&
				...
				if (read_c0_config6() & MIPS_CONF6_EXTIMER)
					val = cop0->reg[rd][sel] | MIPS_CONF6_EXTIMER;
				else
					val = cop0->reg[rd][sel] & (~MIPS_CONF6_EXTIMER);
		...
		case mtc_op:
			...
			if (rd == MIPS_CP0_COUNT &&
				...
				if (val & 0x1000) {
					/* flush VTLB */
					kvm_ls3acomp_clear_guest_vtlb();
				}
				if (val & 0x2000) {
					/* flush FTLB */
					kvm_ls3acomp_clear_guest_ftlb();
				}
				local_irq_restore(flags);
			} else {
				er = EMULATE_FAIL;
			      ...
			      s64 *v)
{
	...
	case KVM_REG_MIPS_COUNTER:
		if(vcpu->kvm->arch.use_stable_timer){
			*v = drdtime() + (signed long)csr_readq(0xfffffff8);
		}else
			*v = csr_readq(LOONGSON_CSR_NODECNT) + vcpu->kvm->arch.nodecounter_offset;
		break;
	default:
		return -EINVAL;
	}
			      ...
			      s64 v)
{
	...
	case KVM_REG_MIPS_COUNTER:
		if (vcpu->kvm->arch.use_stable_timer) {
			...
			csr_writeq((unsigned long)vcpu->kvm->arch.stablecounter_gftoffset, 0xfffffff8);
			local_irq_restore(flags);
		} else if (vcpu->vcpu_id == 0)
			vcpu->kvm->arch.nodecounter_offset = v - csr_readq(LOONGSON_CSR_NODECNT);
		break;
	default:
		return -EINVAL;
	}
...
static int kvm_vz_vcpu_load(struct kvm_vcpu *vcpu, int cpu)
{
	...
	/* Set bit0 for use root.CCA as default */
...
static struct kvm_mips_callbacks kvm_ls3acomp_vz_callbacks = {
...
int kvm_ls3acomp_emulation_init(struct kvm_mips_callbacks **install_callbacks)
{
	...
	*install_callbacks = &kvm_ls3acomp_vz_callbacks;
	return 0;
}

static struct kvm_timer_callbacks kvm_ls3acomp_timer_callbacks = {
	.restore_timer = kvm_ls3acomp_restore_timer,
	.acquire_htimer = kvm_ls3acomp_acquire_htimer,
	.save_timer = kvm_ls3acomp_save_timer,
	.lose_htimer = kvm_ls3acomp_lose_htimer,
...
int kvm_ls3acomp_timer_init(struct kvm_timer_callbacks **install_callbacks)
{
	...
	*install_callbacks = &kvm_ls3acomp_timer_callbacks;
	return 0;
}
